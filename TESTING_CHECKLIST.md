...
- V16 Tony memory engine foundation (`tests/test_outcome_analytics.py`: daily memory summary counts, real-only filtering, demo/legacy exclusion, reassessment-label rollups, raw-history-preserved notes, no deletion/mutation of snapshot history; manual: `eod-report` prints Tony memory summary and does not imply auto-learning).
- V16A ET market-date reporting (`tests/test_outcome_analytics.py`: ET boundary helper, `outcome-analytics --today` uses America/New_York market date, `eod-report` default date and explicit `--date` override align with Tony memory report date; full-suite check required).
