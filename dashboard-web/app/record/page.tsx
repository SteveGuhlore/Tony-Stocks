...
function EquityOverlay({ tony }: { tony: number[] }) {
  if (tony.length < 2) {
    return <p style={{ color: "var(--text-tertiary)", fontSize: 11 }}>Not enough closed trades to chart equity yet.</p>
  }
  const W = 600, H = 120
  const min = Math.min(...tony), max = Math.max(...tony)
  const pts = tony.map((v, i) => `${(i / (tony.length - 1)) * W},${scaleY(v, min, max, H)}`).join(" ")
  return (
    <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" width="100%" height={H} style={{ display: "block", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: 6 }}>
      <polyline points={pts} fill="none" stroke="var(--accent)" strokeWidth={1.6} vectorEffect="non-scaling-stroke" />
    </svg>
  )
}
...
function MatrixCell({ value, label, border }: { value: string; label: string; border: string }) {
  return (
    <div style={{ background: "var(--bg-elevated)", border: `1px solid ${border}`, borderRadius: 6, padding: 8 }}>
      <div className="mono" style={{ fontSize: 16, fontWeight: 700, color: "var(--text-secondary)" }}>{value}</div>
      <div style={{ fontSize: 9, color: "var(--text-tertiary)" }}>{label}</div>
    </div>
  )
}
    ...
    <div style={{ maxWidth: 760 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
        <div className="card">
          ...
          <div style={{ display: "flex", gap: 18 }}>
            ...
            <Stat value={bt?.avg_simulated_pl_per_trade != null ? bt.avg_simulated_pl_per_trade.toFixed(2) : "—"} label="avg P/L" />
        ...
        <div className="card">
          ...
          <div style={{ display: "flex", gap: 18 }}>
            <Stat value="—" label="win rate" color="var(--text-tertiary)" />
            <Stat value="—" label="avg P/L" color="var(--text-tertiary)" />
            <Stat value="—" label="tgt hits" color="var(--text-tertiary)" />
            <Stat value="—" label="stop hits" color="var(--text-tertiary)" />
          </div>
          <div style={{ fontSize: 9, color: "var(--text-tertiary)", marginTop: 8 }}>⋯ awaiting Command Center record</div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          ...
          <span className="mono" style={{ fontSize: 10 }}><span style={{ color: "var(--accent)" }}>▬ Tony</span> <span style={{ color: "var(--text-tertiary)" }}>▬ Tony Stocks (awaiting)</span></span>
        </div>
        <EquityOverlay tony={equity} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
        <div className="card">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <MatrixCell value="—" label="agreed & right" border="var(--border)" />
            <MatrixCell value="—" label="agreed & wrong" border="var(--border)" />
            <MatrixCell value="—" label="CC overrode & saved" border="var(--border)" />
            <MatrixCell value="—" label="CC overrode & missed" border="var(--border)" />
          </div>
          <p style={{ fontSize: 10, color: "var(--text-tertiary)", marginTop: 8 }}>Populates once the Command Center returns verdicts.</p>
        </div>
        <div className="card">
